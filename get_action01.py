...
##Two options specifically for functionalization of dopants
flag_dopfun = True
...
def check_dopant(df, dop_type=23):


    #This function checks the existence of dopants in the CNT-structure
    df_d = df[df['type']==dop_type]
...
def drop_existing_bonds(df, init_file, file_num, threshold=2.0, type_choose=22):
    ...
    if file_num > 0:
        ...
        if type_choose == 23:
            ...
            #if we are functionalizing dopants, we need to drop dopants as a functional group
            used_fun = used_fun[used_fun != 5]
        ...
        for num in used_fun:
            ...
            #Troubleshoot
            #check for dopant nitrogen bonding
            #d_cnt
...
def define_action_space(a, r_pick, R_space=[2.25, 2.75, 3.25, 3.75], action_space=np.arange(0,3)):

    #Rspace was discretized

    #define action matrix:
    A = np.zeros((len(R_space, action_space)))
...
def pick_group_and_atom(df, type_choose):
    ...
    for fun in range(len(option_list)):
        _, _, mat_1 = detect_locations(df=df, type_choose=type_choose, threshold=4.0, fun_type=fun)
        ...
        for i in range(0, len(mat_1)):
            ...
            A = define_action_space(a=fun, r_pick=D)



        if fun == 12:
            H_type = 1
        elif fun == 9 or fun == 15:
            H_type = 11
...
def set_action(file_to_read, init_file, file_num):
    df = read_datafile(file_to_read)
    bool_d = check_dopant(df)
...
while H_id == 0:
    ...
    try:
        cnt_id, fun_id, R_id, H_id, c_id = pick_group_and_atom(df, option_fun=option_fun, minimize=False, type_choose=type_choose)
