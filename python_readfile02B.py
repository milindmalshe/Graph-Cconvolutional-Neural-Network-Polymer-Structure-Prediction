...
def pick_group_and_atom(df, **kwargs):
    ...
    if 'type_choose' in kwargs and kwargs['type_choose'] == 23:
        type_choose = 23
    else:
        type_choose = 22

    df_fun, df_cnt, mat_1 = detect_locations(df=df, type_choose=type_choose, threshold=4.0, fun_type=fun_type)
...
#block to check if dopant functionalization is "on"
if bool_d == False:
    type_choose = 23
else:
    type_choose = 22
...
#drop existing bonds:
df = drop_existing_bonds(df, init_file=init_file, file_num=file_num, type_choose=type_choose)
H_id = 0
