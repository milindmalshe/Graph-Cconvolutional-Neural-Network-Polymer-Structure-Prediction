...
#-----------------------------------------------------------------------------
def feature_equation_02(r, cnt_mat, other_mat, other_type, unique_type=np.asarray([2, 4, 9, 12, 15, 18, 23]), type_choose=22):
...
def CNT_atoms(df, z_len, type_choose=22):
    ...
    RBF_array, r_v = compute_RDF(df_cnt=df_cnt, df_other=df_other, z_len=z_len, bounds=np.asarray([1.0, 2.0, 8.0]))
...
def discretize_2D(cnt_array, RBF_array, center_array= np.asarray([30.0, 30.0]), grid = np.asarray([4.0, 2.0]), x_min=25.5, x_max = 35.5, z_min = 8.0, z_max = 52.0):
...
def RBF_Setter(df_list, z_list, total_files):
...
def RBF_Setter02(df_list, z_list, total_files):
    ...
    for i in range(len(df_list)):
        ...
        RBF, r_v = CNT_atoms(df_list[i], z_list[i])
        r_d, RBF_d = discretize_features(RBF=RBF, new_intv=[0.2, 0.2, 0.2], bounds=np.asarray([1.0, 2.0, 8.0]))
...
def partition_data03(X, X2, Y, n, n2, f=400):
    ...
    X_t1, X_t2, Y_t = X[:N, :, :, :], X2[:N, :, :, :], Y[:N]
    X_v1, X_v2, Y_v = X[N:N2, :, :, :], X2[N:N2, :, : :], Y[N:N2]
    X_e1, X_e2, Y_e = X[N2:, :, :, :], X2[N2:, :, :, :], Y[N2:]


    return X_t1, X_t2, Y_t, X_e1, X_e2, Y_e, X_v1, X_v2, Y_v
